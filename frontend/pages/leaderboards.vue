    ...
    <article class="leaderboard">
      <header>
        ...
        <h1 class="leaderboard__title">
          <span class="leaderboard__title--top">Pong</span><span class="leaderboard__title--bottom">Leaderboard</span>
        </h1>
      </header>

      <main class="leaderboard__profiles">
        <article v-for="item in users" :key="item.user.id" class="leaderboard__profile" @click="$router.push({ name: 'profile', params: { id: item.user.id } })">
          <img :src="'/api/uploads/' + item.user.id + '.png'" alt="Mark Zuckerberg" class="leaderboard__picture">
          <span class="leaderboard__name">{{ item.user.username }}</span>
          <span class="leaderboard__value">{{ item.rating }}<span>MMR</span></span>
        </article>
      </main>
    </article>
  </div>
